    ...
    >
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/80 shadow-lg">
        <div className="flex items-center space-x-4 mb-4">
          <div className="w-12 h-12 rounded-full bg-gradient-to-r from-sa-gold-500 to-sa-gold-600 flex items-center justify-center shadow-lg">
            <step.icon className="w-6 h-6 text-white" />
          </div>
          <div className="flex-1">
            <div className="text-sm font-medium text-sa-gold-600 mb-1">Step {index + 1}</div>
        ...
        <div className="space-y-2">
          {step.features.map((feature: string, idx: number) => (
            <div key={idx} className="flex items-center space-x-2">
              <div className="w-1.5 h-1.5 bg-sa-gold-500 rounded-full" />
      ...
      {index < steps.length - 1 && (
        <div className="flex justify-center my-6">
          <ArrowRight className="w-6 h-6 text-sa-gold-400" />
        </div>
      )}
    </motion.div>
  );
    ...
    return (
        ...
        >
                    ...
                    <motion.div variants={cardVariants} className={`w-full ${isReversed ? 'pl-8' : 'pr-8'}`}>
                        <div className="bg-white/90 backdrop-blur-sm p-6 rounded-2xl border border-slate-200/80 shadow-lg hover:shadow-xl transition-shadow duration-300">
                            <div className="flex items-center space-x-3 mb-3">
                              <div className="text-sm font-medium text-sa-gold-600">Step {index + 1}</div>
                            </div>
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 h-full flex items-start justify-center">
                 <motion.div variants={markerVariants} className="relative w-12 h-12 rounded-full bg-gradient-to-r from-sa-gold-500 to-sa-gold-600 flex items-center justify-center border-4 border-white shadow-lg z-10">
                    <step.icon className="w-6 h-6 text-white" />
                    <motion.div variants={markerGlowVariants} className="absolute w-full h-full bg-sa-gold-500 rounded-full blur-lg" />
                </motion.div>
            </div>
        </motion.div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={containerRef} className="hidden lg:block relative max-w-4xl mx-auto">
            ...
            <motion.div 
                ...
                className="absolute left-1/2 -translate-x-1/2 top-6 w-1 bg-gradient-to-b from-sa-gold-500 to-sa-gold-600" 
            />
